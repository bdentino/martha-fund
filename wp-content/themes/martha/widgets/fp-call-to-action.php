...
class Martha_Action extends WP_Widget {

    function martha_action() {
		$widget_ops = array('classname' => 'martha_action_widget', 'description' => __( 'Display a call to action block.', 'martha') );
        parent::__construct(false, $name = __('Martha FP: Call to action', 'martha'), $widget_ops);
		$this->alt_option_name = 'martha_action_widget';
    }
	...
	?>
	<p><label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'martha'); ?></label>
	<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" /></p>
	<p><label for="<?php echo $this->get_field_id('action_text'); ?>"><?php _e('Enter your call to action.', 'martha'); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id('action_btn_link'); ?>"><?php _e('Link for the button', 'martha'); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id('action_btn_text'); ?>"><?php _e('Title for the button', 'martha'); ?></label>
	...
	<label for="<?php echo $this->get_field_id( 'inline' ); ?>"><?php _e( 'Display the button inline with the text?', 'martha' ); ?></label></p>
	<?php
	}
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['martha_action']) )
			delete_option('martha_action');
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'martha_action', 'widget' );
		}
...
?>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'martha_action', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
