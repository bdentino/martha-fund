...
class Martha_Employees extends WP_Widget {

    function martha_employees() {
		$widget_ops = array('classname' => 'martha_employees_widget', 'description' => __( 'Display your team members in a stylish way.', 'martha') );
        parent::__construct(false, $name = __('Martha FP: Employees', 'martha'), $widget_ops);
		$this->alt_option_name = 'martha_employees_widget';
	...
	<p><?php _e('In order to display this widget, you must first add some employees from the dashboard. Add as many as you want and the theme will automatically display them all.', 'martha'); ?></p>
	<p>
	<label for="<?php echo $this->get_field_id('title'); ?>"><?php _e('Title', 'martha'); ?></label>
	<input class="widefat" id="<?php echo $this->get_field_id('title'); ?>" name="<?php echo $this->get_field_name('title'); ?>" type="text" value="<?php echo $title; ?>" />
	</p>
	<p><label for="<?php echo $this->get_field_id( 'number' ); ?>"><?php _e( 'Number of employees to show (-1 shows all of them):', 'martha' ); ?></label>
	<input id="<?php echo $this->get_field_id( 'number' ); ?>" name="<?php echo $this->get_field_name( 'number' ); ?>" type="text" value="<?php echo $number; ?>" size="3" /></p>
    <p><label for="<?php echo $this->get_field_id('see_all'); ?>"><?php _e('Enter an URL here if you want to section to link somewhere.', 'martha'); ?></label>
    ...
    <p><label for="<?php echo $this->get_field_id('see_all_text'); ?>"><?php _e('The text for the button [Defaults to <em>See all our employees</em> if left empty]', 'martha'); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id( 'category' ); ?>"><?php _e( 'Enter the slug for your category or leave empty to show all employees.', 'martha' ); ?></label>
	...
	<label for="<?php echo $this->get_field_id( 'center_content' ); ?>"><?php _e( 'Center the employees? (use only if you have 1 or 2 employees)', 'martha' ); ?></label></p>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['martha_employees']) )
			delete_option('martha_employees');
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'martha_employees', 'widget' );
		}
...
?>
		...
		<div class="roll-team carousel owl-carousel">
			...
			<div class="team-item">
			    <div class="team-inner">
					...
					<div class="avatar">
						<?php the_post_thumbnail('martha-medium-thumb'); ?>
					</div>
					<?php endif; ?>
			    </div>
		...
		<?php if ($see_all != '') : ?>
			<a href="<?php echo esc_url($see_all); ?>" class="roll-button more-button">
				...
				<?php else : ?>
					<?php echo __('See all our employees', 'martha'); ?>
				<?php endif; ?>
			</a>
		<?php endif; ?>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'martha_employees', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
