...
class Martha_Facts extends WP_Widget {

// constructor
    function martha_facts() {
		$widget_ops = array('classname' => 'martha_facts_widget', 'description' => __( 'Show your visitors some facts about your company.', 'martha') );
        parent::__construct(false, $name = __('Martha FP: Facts', 'martha'), $widget_ops);
		$this->alt_option_name = 'martha_facts_widget';
    }
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['martha_facts']) )
			delete_option('martha_facts');
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'martha_facts', 'widget' );
		}
...
?>
	...
	<?php
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'martha_facts', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
