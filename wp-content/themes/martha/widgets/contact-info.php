...
class Martha_Contact_Info extends WP_Widget {

    function martha_contact_info() {
		$widget_ops = array('classname' => 'martha_contact_info_widget', 'description' => __( 'Display your contact info', 'martha') );
        parent::__construct(false, $name = __('Martha: Contact info', 'martha'), $widget_ops);
		$this->alt_option_name = 'martha_contact_info';
    }
	...
	<p><label for="<?php echo $this->get_field_id( 'address' ); ?>"><?php _e( 'Enter your address', 'martha' ); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id( 'phone' ); ?>"><?php _e( 'Enter your phone number', 'martha' ); ?></label>
	...
	<p><label for="<?php echo $this->get_field_id( 'email' ); ?>"><?php _e( 'Enter your email address', 'martha' ); ?></label>
	...
	function update($new_instance, $old_instance) {
		...
		if ( isset($alloptions['martha_contact_info']) )
			delete_option('martha_contact_info');
	...
	function widget($args, $instance) {
		$cache = array();
		if ( ! $this->is_preview() ) {
			$cache = wp_cache_get( 'martha_contact_info', 'widget' );
		}
		...
		if ( ! $this->is_preview() ) {
			...
			wp_cache_set( 'martha_contact_info', $cache, 'widget' );
		} else {
			ob_end_flush();
		}
	}
